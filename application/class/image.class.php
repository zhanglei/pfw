...
class Image {
	...
	public function exceptionFunc($msg) {
	...
	// 获取图片信息
	public function getImgInfo($key = false) {
		if ($key) {
	 ...
	 */
	public function wave($grade = 5, $dir = "h") {
		$w = $this -> getWidth();
		$h = $this -> getHeight();
		if ($dir == "h") {
			for ($i = 0; $i < $w; $i += 2) {
	 ...
	 * @return
	 */
	public function waterMark($markImg, $hp = 'center', $vp = 'center', $pct = 50, $path = null) {
		//原图信息
		$srcw = $this -> getWidth();
		$srch = $this -> getHeight();
